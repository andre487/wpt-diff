...
module.exports = function runWptDiff(params) {
    ...
    return differ.run(params.urls, params.labels)
        .then(function (data) {
...
_.assign(WPTDiff.prototype, /** @lends WPTDiff.prototype */ {
    ...
    run: function (urls, labels) {
        ...
        return this._executePipeline(urls, labels);
    },

    _executePipeline: function (urls, labels) {
        return this.runTests(urls, labels)
     ...
     */
    runTests: function (urls, labels) {
        logger.info('[differ] Run tests for %s', urls.join(', '));

        return this._launchTests(urls, labels)
    ...
    _launchTests: function (urls, labels) {
        var testPromises = urls.map(function (url, i) {
            var options = _.cloneDeep(this._testOptions);
            labels.length && (options.label = labels[i]);

            logger.info('[differ] Launch test for ' + url + ' with params:', options);
            return Q.ninvoke(this._wpt, 'runTest', url, options);
        }, this);
    ...
    _rememberTestsData: function (results) {
        ...
        var testsInfo = this._results.testCases = {};
    ...
    createVideo: function (testsDataList) {
        ...
        logger.info('[differ] Create video for %s', testIds);
        ...
        function processData(videoData) {
            ...
            logger.info('[differ] Video for %s complete, its id: %s', testIds, data.videoId);

            that._results.video = data;
            return videoData;
        }

        return Q.ninvoke(this._wpt, 'createVideo', testIds, {})
            .then(prepareWptResult)
            .then(checkStatus)
            .then(processData);
    },
