...
var ENV_MAPPING = {
    ...
    WPT_BODIES: {name: 'bodies', type: _.flow(Number, Boolean)},
    WPT_TIMELINE: {name: 'timeline', type: _.flow(Number, Boolean)},
...
parameters.get = function getProgramParameters() {
    ...
    if (params.labels.length > 0 && params.labels.length != params.urls.length) {
...
parameters.getFromCli = function getProgramParametersFromCli(argv) {
    ...
    argParser.addArgument(
        ['--location'],
        {dest: 'location', help: 'Test server location, see https://clck.ru/9fn76. Env: WPT_LOCATION'}
    );
    ...
    argParser.addArgument(
        ['--timeline'],
        {dest: 'timeline', action: 'storeTrue', help: 'Save timeline. Env: WPT_TIMELINE'}
    );

    argParser.addArgument(
        ['--bodies'],
        {dest: 'bodies', action: 'storeTrue', help: 'Save text resources bodies. Env: WPT_BODIES'}
    );
